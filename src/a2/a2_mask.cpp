...
class state_t
{
    public:
        ...
        vx_mouse_event_t    last_two_mouse_event;
    ...
    public:
        ...
        static int mouse_event (vx_event_handler_t *vxeh, vx_layer_t *vl, vx_camera_pos_t *pos, vx_mouse_event_t *mouse)
        {
            ...
            if ((mouse->button_mask & VX_BUTTON1_MASK) &&
                ...
                printf ("Mouse clicked at coords: [%8.3f, %8.3f]\n",
                        mouse->x, mouse->y);
                if(state->click_count == 0){
                    state->last_two_mouse_event = *mouse;
                    state->click_count += 1;
                }
                    ...
                    state->corner_coords[1].x = mouse->x;
                    state->corner_coords[1].y = mouse->y;
                    state->click_count += 1;
                }
                else{
                    state->click_count = 0;
