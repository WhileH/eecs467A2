...
class state_t
{
	...
	public:
		...
		static void* render_loop(void* data)
		{
			...
			while (state->running) {
				...
				if(state->corner_coords[0].x != -1 && state->corner_coords[0].y != -1 && state->corner_coords[1].x != -1 && state->corner_coords[1].y != -1){  
					...
					red_center_list = state->im_processor.blob_detection(im,state->corner_coords[0].x,state->corner_coords[1].x,state->corner_coords[0].y,state->corner_coords[1].y,state->cal.get_red());
					green_center_list = state->im_processor.blob_detection(im,state->corner_coords[0].x,state->corner_coords[1].x,state->corner_coords[0].y,state->corner_coords[1].y,state->cal.get_green()); 
		...
		static void display_started(vx_application_t * app, vx_display_t * disp)
		{
			...
			std::vector<float> xys = state->cal.get_mask();
